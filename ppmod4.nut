  ...
  return ppromise(function (resolve, reject):(type, pos, ang, model) {
    ...
    ppmod.create(model).then(function (ent):(type, pos, ang, resolve) {
      ...
      if (type == "prop_button" || type == "prop_under_button") { // Handle pedestal buttons
        ...
        ppmod.keyval(unpressrl,"targetname","ppmod_unpressrl")
        ...
        local unpress = function ():(type, trigger, ent) {
          if (--trigger.GetScriptScope()["count"] == 0) {
            ...
            ppmod.fire("ppmod_unpressrl", "Trigger", "", 0.0, null, null)
...
::ppmod.catapultex <- function (ent, vec, speed = false, enabled = true,cstatic = false,spawnflags = 8,sizex = Vector(-0.2,-0.2,-0.2),sizey = Vector(0.2,0.2,0.2)) {

  if (!(typeof ent == "instance" && ent instanceof CBaseEntity)) {
    ppmod.getall(ent, function (curr):(vec,speed,enabled,cstatic,spawnflags,sizex,sizey) {
      ppmod.catapultex(curr, vec, speed, enabled, cstatic, spawnflags, sizex, sizey);
    });
    return;
  }
  ...
  if (speed == false) {
    trigger.__KeyValueFromFloat("PhysicsSpeed", vec.Norm());
  } else {
    trigger.__KeyValueFromFloat("PhysicsSpeed", speed);
  }
