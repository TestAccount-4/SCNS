...
function include_start() {
  ...
  if(GetMapName() == "sp_a4_finale4") {
    if(!ppmod.get("button_model") && ppmod.get("viewmodel")) rev.spawn();
    else delete rev;
    return;
  }
}
...
reverse_maps["sp_a1_intro1"] <- function() {
  rev.setup <- function() {
    ...
    // Fadeout button
    SendToConsole("ent_create prop_button");

    local auto = Entities.CreateByClassname("logic_auto");
    ppmod.addscript(auto, "OnLoadGame", "rev.saveload()");
    SendToConsole("save autosave; load autosave");
  }

  rev.saveload <- function() {
  ...
  rev.level.button <- function() {
    local button = ppmod.get("prop_button");
    button.SetOrigin(Vector(-1692, 4325, 3081));
    ppmod.fire(button, "DisableDraw");
...
reverse_maps["sp_a3_01"] <- function() {
  rev.setup <- function() {
    ...
    EntFire("knockout-portalgun","kill");
    SendToConsole("give weapon_portalgun");
  ...
  rev.loop <- function() {
    ...
    if(ppmod.trigger(-1412.71, -163.747, -6139.97, -1341.17, -92.6758, -6097.51, null, null, true)){
    	if(rev.level.solve >= 3) {
    		...
    		if(rev.level.solve != -1) ScriptShowHudMessageAll("This level has to be unsolved to proceed!", 0.0334);
    	}
    }
  }
...
reverse_maps["sp_a4_finale2"] <- function() {
  rev.setup <- function() {
  	...
  	ppmod.keyval(trigger, "SpawnFlags", 1);
  	ppmod.fire(trigger, "Enable");
  	ppmod.addscript(trigger, "OnStartTouch", "rev.level.funnelclip()");
  ...
  rev.level.funnelclip <- function() {
  	...
  	for(local i = 0; i < 20; i++) {
  		ppmod.wait("GetPlayer().SetOrigin(GetPlayer().GetOrigin()+Vector("+vel+",0,0))", FrameTime()*i);
  	}
  }
...
reverse_maps["sp_a4_finale4"] <- function() {
  rev.setup <- function() {
  	EntFire("ending_vehicle", "unlock");
  	EntFire("ending_vehicle", "exitvehicle");
  	EntFire("ending_gun_model", "kill");
  	EntFire("moon_portal_brush", "kill");
  	EntFire("!player", "SetLocalOrigin", "529.334 73.915 1", 0.1);
  	SendToConsole("snd_restart");
  }
...
if("Entities" in this) {
	...
	printl("\n=== Loading Reverse mod...");
	printl("=== Version: release-1.2, ppmod2");
